...
public class SqlTests
{
    ...
    [TestOrder(42)]
    [Fact]
    [DisplayName("SQLite查询指定列名格式化")]
    public void SelectWithColumnsSQLite()
    {
        // 测试单列：属性名"Name"在SQLite下保持原样（非保留字）
        var exp = User._.Name == "Stone";
        var builder = User.CreateBuilder(exp, null, "Name");
        var sql = builder.ToString();
        Assert.Equal(@"Select Name From User Where Name='Stone'", sql);
    }

    [TestOrder(42)]
    [Fact]
    [DisplayName("MySql下划线格式查询指定列名转换")]
    public void SelectWithColumnsMySqlUnderline()
    {
        // 测试单列：MySQL下划线格式下，属性名"Name"应格式化为"name"（小写下划线）
        using var split = User.Meta.CreateSplit("mysql_underline", null);
    ...
    [TestOrder(42)]
    [Fact]
    [DisplayName("MySql下划线格式查询多列名转换")]
    public void SelectWithMultipleColumnsMySqlUnderline()
    {
        // 测试多列：MySQL下划线格式下，多个属性名应各自格式化
        using var split = User.Meta.CreateSplit("mysql_underline", null);

        var exp = User._.Name == "Stone";
        var builder = User.CreateBuilder(exp, null, "Name,DisplayName");
        var sql = builder.ToString();
        Assert.Equal(@"Select name,display_name From `user` Where name='Stone'", sql);
    }

    [TestOrder(42)]
    [Fact]
    [DisplayName("SQL表达式不被格式化")]
    public void SelectWithSqlExpressionPreserved()
    {
        // 测试SQL表达式（含括号）不应被格式化处理，直接透传
        var exp = User._.Name == "Stone";
        var builder = User.CreateBuilder(exp, null, "Count(*) as Count");
        var sql = builder.ToString();
        Assert.Equal(@"Select Count(*) as Count From User Where Name='Stone'", sql);
    }

    [TestOrder(42)]
    [Fact]
    [DisplayName("混合列名与表达式格式化")]
    public void SelectWithMixedColumnsAndExpression()
    {
        // 测试混合场景：简单列名与SQL表达式混合，列名格式化，表达式透传
        using var split = User.Meta.CreateSplit("mysql_underline", null);

        var exp = User._.Name == "Stone";
        var builder = User.CreateBuilder(exp, null, "Name,Count(*) as Total");
        var sql = builder.ToString();
        // Name 格式化为 name，SQL 表达式 Count(*) as Total 透传
        Assert.Equal(@"Select name,Count(*) as Total From `user` Where name='Stone'", sql);
    }
