...
public class SqlTests
{
    ...
    public void InsertTestSQLite()
    {
        ...
        Assert.Equal(@"Insert Into User(Name,DisplayName,Sex,AreaId,RoleID,DepartmentID,Online,Enable,Age,Logins,RegisterTime,OnlineTime,Ex1,Ex2,Ex3,UpdateUserID,UpdateTime) Values('Stone','大石头',0,0,3,0,0,1,0,0,'2020-08-22 00:00:00',0,0,0,0,0,'2020-09-01 00:00:00')", sql);
    }
    ...
    public void InsertTestMySqlUnderline()
    {
        ...
        var sql = factory.Persistence.GetSql(session, user, DataObjectMethodType.Insert);
        Assert.Equal(@"Insert Into `user`(name,display_name,sex,area_id,role_id,department_id,online,enable,age,logins,register_time,online_time,ex1,ex2,ex3,update_user_id,update_time) Values('Stone','大石头',0,0,3,0,0,1,0,0,'2020-08-22 00:00:00',0,0,0,0,0,'2020-09-01 00:00:00')", sql);
    }
    ...
    public void ShardTestSQLite2()
    {
        ...
        Assert.StartsWith($"[test_{time:yyyy}] Select * From Log2_{time:yyyyMMdd} Where", sqls[^3]);
        ...
        Assert.StartsWith($"[test_{time:yyyy}] Select * From Log2_{time:yyyyMMdd} Where", sqls[^3]);
        Assert.StartsWith($"[test_{time:yyyy}] Select Count(*) From Log2_{time:yyyyMMdd} Where", sqls[^2]);
        Assert.StartsWith($"[test_{time:yyyy}] Select * From Log2_{time.AddHours(-24):yyyyMMdd} Where", sqls[^1]);
